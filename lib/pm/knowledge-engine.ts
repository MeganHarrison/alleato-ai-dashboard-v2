...
export class PMKnowledgeEngine {
   ...
   */
  private extractStatusIndicators(chunks: any[]): Record<string, number> {
   ...
   */
  private extractRisks(chunks: any[]): string[] {
    const risks: string[] = [];
   ...
   */
  private extractActionItems(chunks: any[]): PMAnalysis['actionItems'] {
  ...
  private extractStakeholderInsights(context: MeetingContext): string[] {
    ...
    context.chunks.forEach(chunk => {
      if (chunk.speakers && Array.isArray(chunk.speakers)) {
        chunk.speakers.forEach((speaker: any) => {
