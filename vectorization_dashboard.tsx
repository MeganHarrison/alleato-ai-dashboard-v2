...
import { Loader2, Play, RefreshCw, Database, FileText, CheckCircle, AlertCircle, XCircle } from 'lucide-react';
...
interface Document {
  ...
  chunk_count: number;
  status: 'unprocessed' | 'completed' | 'partially_embedded' | 'insufficient_content';
  created_at: string;
}
...
export default function VectorizationDashboard() {
  ...
  const [lastResult, setLastResult] = useState<VectorizationResult | null>(null);
  ...
  const triggerVectorization = async (documentId?: string, dryRun = false) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to trigger vectorization');
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage document embeddings and vector search setup</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.documents_processed}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.documents_unprocessed}</div>
              <p className="text-xs text-muted-foreground">
                Need vectorization
              </p>
            </CardContent>
              ...
              <AlertDescription>
                <div className="space-y-2">
                  <div className="font-medium">
                    Processing Result: {lastResult.success ? 'Success' : 'Failed'}
                  </div>
                  <div className="text-sm">
                    Found: {lastResult.documentsFound} documents | 
                    Processed: {lastResult.documentsProcessed} | 
                    Successes: {lastResult.successes} | 
                  ...
                  <div className="flex items-center space-x-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm text-gray-500">
                        {doc.project_name && <span className="mr-3">{doc.project_name}</span>}
                        <span className="mr-3">{doc.document_type}</span>
                        <span>{(doc.content_length / 1000).toFixed(1)}k chars</span>
                        {doc.chunk_count > 0 && <span className="ml-3">{doc.chunk_count} chunks</span>}
                      </div>
                    </div>
                  </div>
