...
class FMDocsClient {
  ...
  async getSectionNavigation(slug: string): Promise<{
    ...
    const { data, error } = await this.supabase.rpc("get_section_navigation", {
      current_slug: slug,
    });
    if (error) throw error;
  ...
  const renderBlock = (block: Block) => {
    ...
    return (
      <div key={block.id} id={blockId} className="mb-6 scroll-margin-top-20">
        ...
        {(block.inline_figures?.length > 0 ||
          block.inline_tables?.length > 0) && (
  ...
  return (
    <article className="max-w-4xl">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          {content.page_start && (
            <div className="text-sm text-gray-600">
              ...
              {content.page_end &&
                content.page_end !== content.page_start &&
                `-${content.page_end}`}
            </div>
          )}
        </div>
      ...
      <div className="space-y-6">
        {content.blocks.sort((a, b) => a.ordinal - b.ordinal).map(renderBlock)}
      </div>
...
export const FMDocsApp: React.FC<FMDocsAppProps> = ({
  ...
  const [currentContent, setCurrentContent] = useState<SectionContent | null>(
    null
  );
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center max-w-md">
                ...
                <p className="text-sm text-gray-500">
                  Select a section from the sidebar to begin reading, or use the
                  search bar to find specific information.
                </p>
              </div>
            </div>
          )}
